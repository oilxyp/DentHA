                                ...
                                <table id="example" class="table-hover whitespace-nowrap" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr style="text-align: center">
                                            <td>{{ $row->id }}</td>
                                            <td>{{ $row->name }}</td>                               
                                            <td style="max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ $row->detail }}</td>
                                            <td>
                                                <div class="flex items-center justify-center gap-4">
                                                    <a href="{{ route('vision_manage.edit', $row->id) }}"
                                                        class="btn btn-sm btn-outline-warning"> แก้ไข</a>
                                                    {{-- <form action="{{ route('vision_manage.destroy', $row->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
