...
@section('title', 'แก้ไขข้อมูลสถิติ')
@section('content')
                    ...
                    <div class="panel p-5">
                        ...
                        @if ($source === 'elderlygroup')
                            <h4 class="text text-center mb-6 text-xl font-bold md:text-1xl">
                                แก้ไขข้อมูลสถิติผู้ป่วยกลุ่มผู้สูงอายุ</h4>
                            <form method="POST"
                                action="{{ route('record_manage.update', ['record_manage' => $data->id, 'source' => 'elderlygroup']) }}">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-5">
                                    ...
                                    <select id="year" name="year" class="form-input" value="{{ $data->year }}">
                                        <?php
                                        $current_year = date('Y');
                                ...
                                <div class="mb-2" style="display: flex; justify-content: space-between;">
                                    <div style="flex-basis: 28%;">
                                        ...
                                        <input type="text" id="HN" name="HN" rows="3" placeholder="HN"
                                            class="form-input" style="display: inline-block;"
                                            value="{{ $data->HN }}"></input>
                                    </div>
                                    ...
                                    <div style="flex-basis: 50%;">
                                        ...
                                        <input type="text" id="name" name="name" rows="3"
                                            placeholder="ชื่อ-นามสกุล" class="form-input" style="display: inline-block;"
                                            value="{{ $data->name }}"></input>
                                    </div>
                                    <div style="flex-basis: 18%;">
                                        <label for="old">อายุ</label>
                                        <input type="nunber" id="old" name="old" rows="3"
                                            placeholder="อายุ" class="form-input" value="{{ $data->old }}"></input>
                                    </div>
                                </div>
                                ...
                                <div class="mb-5">
                                    ...
                                    <input type="text" id="congenital_disease" name="congenital_disease" rows="3"
                                        placeholder="โรคประจำตัว" class="form-input"
                                        value="{{ $data->congenital_disease }}"></input>
                                </div>

                                <div class="mb-5">
                                    <label for="medium_dependence">medium_dependence</label>
                                    <input type="text" id="medium_dependence" name="medium_dependence" value="1"
                                        rows="3" placeholder="medium_dependence" class="form-input"></input>
                                </div>
                                ...
                                <div class="mb-2" style="display: flex; justify-content: space-between;">
                                    ...
                                    <div class="mt-8 flex items-center justify-end">
                                        <button type="reset" class="btn btn-outline-danger">
                                            <a href="{{ route('record_manage.index') }}">ยกเลิก</a>
                                        </button>
                        ...
                        @elseif ($source === 'specialpatientgroup')
                            <h4 class="text text-center mb-6 text-xl font-bold md:text-1xl">
                                แก้ไขข้อมูลสถิติผู้ป่วยกลุ่มเด็กพิเศษที่มีอายุ 12 ปี ขึ้นไป</h4>
                                <form method="POST"
                                action="{{ route('record_manage.update', ['record_manage' => $data->id, 'source' => 'specialpatientgroup']) }}">
                                @csrf
                                @method('PUT')
                                <div class="mb-5">
                                    <label for="year">ปี</label>
                                ...
                                <div class="mb-2" style="display: flex; justify-content: space-between;">
                                    <div style="flex-basis: 28%;">
                                        <label for="HN" style="display: inline-block;">HN</label>
                                        <input type="text" id="HN" name="HN" value="{{ $data->HN }}" rows="3"
                                            placeholder="HN" class="form-input" style="display: inline-block;"></input>
                                    </div>

                                    <div style="flex-basis: 50%;">
                                        <label for="name" style="display: inline-block;">ชื่อ-นามสกุล</label>
                                        <input type="text" id="name" name="name" value="{{ $data->name }}" rows="3"
                                            placeholder="ชื่อ-นามสกุล" class="form-input"
                                            style="display: inline-block;"></input>
                                    </div>
                                    <div style="flex-basis: 18%;">
                                        <label for="old">อายุ</label>
                                        <input type="nunber" id="old" name="old" value="{{ $data->old }}" rows="3"
                                            placeholder="อายุ" class="form-input"></input>
                                    </div>
                                </div>
                                <div class="mb-5">
                                    <label for="detail">เบอร์โทรศัพท์</label>
                                    <input id="phon" name="phone" value="{{ $data->phone }}" type="text" placeholder="เบอร์โทรศัพท์" class="form-input"/>
                                </div>
                                <div class="mb-5">
                                ...
                                <div class="mb-5">
                                    ...
                                    <input type="text" id="U_D"  name="U_D" value="{{ $data->U_D }}" placeholder="โรคประจำตัว" class="form-input"
                                       />
                                </div>
                                <div class="mb-5">
                                    <label for="detail">อื่นๆ</label>
                                    <input type="text" id="GA" name="GA" value="{{ $data->GA }}"  placeholder="อื่นๆ" class="form-input"/>
                                </div>
                        ...
                        @elseif ($source === 'emergency')
                            <h4 class="text text-center mb-6 text-xl font-bold md:text-1xl">
                                แก้ไขข้อมูลสถิติผู้ป่วยกลุ่ม Emergency</h4>
                            <form method="POST"
                                action="{{ route('record_manage.update', ['record_manage' => $data->id, 'source' => 'emergency']) }}">
                                @csrf
                                @method('PUT')
                                <div class="mb-2" style="display: flex; justify-content: space-between;">
                                    <div style="flex-basis: 32%;">
                                        <label for="date" style="display: inline-block;">วันที่</label>
                                        <input type="text" id="date" name="date" rows="3" value="{{$data->date}}"
                                            placeholder="วันที่" class="form-input"
                                            style="display: inline-block;"></input>
                                    </div>
                                    <div style="flex-basis: 32%;">
                                        <label for="year">เดือน</label>
                                        <select id="month" name="month"  value="{{$data->month}}" class="form-input">
                                    ...
                                    <div style="flex-basis: 32%;">
                                        ...
                                        <select id="year" name="year" value="{{$data->year}}" class="form-input">
                                            <?php
                                            $current_year = date('Y');
                                            for ($i = $current_year; $i >= $current_year - 4; $i--) {
                                                echo "<option value=\"$i\">$i</option>";
